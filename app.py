...
def signup():
    if request.method == 'POST':
        ...
        insert=cur.execute("INSERT INTO pdcsignin(stuname, stuemail,stupswd) VALUES(%s, %s, %s)",(email,name,pswd))
        if(insert):
...
def login():
 ...
 if request.method == 'POST':
     resultValue = cur.execute("Select * from pdcsignin")
     if resultValue > 0:
        ...
        result=cur.execute("SELECT * FROM pdcsignin WHERE stuname=%s AND stupswd=%s", (email, pswd)) #parameterized query
        ...
        if(result):
          return render_template('placementmaterials.html')
        else:
...
def login():
    ...
    if request.method == 'POST':
        ...
        show = cur.execute("SELECT * FROM pdcsignin WHERE stuemail = %s", (email,))
        ...
        if(show):
           ...
           # return redirect('/events_workshops')
            return render_template('signup.html')
        else:
            return render_template('events_workshops.html')
...
@app.route('/past_secys')
def past_secys():
      
    return render_template('past_secys.html')


@app.route('/events_workshops')
